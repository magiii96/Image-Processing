...
for c = 1:tcols
   for r = 1:trows
       ...
       if(Omega_2(r,c) == 1)
            ...
            %In the importing gradient, for all<p,q>,
            ...
            x = r-target_x+startpoint(1);
            y = c-target_y+startpoint(2);
           
            %The up neighbour 
            
            if(abs(knownf(r,c) -knownf(r-1,c))) > (abs(source_I(x,y) -source_I(x-1,y)))
                Vpq1 = knownf(r,c) -knownf(r-1,c);
            else
            ...
            %The left neighbour  
            if(abs(knownf(r,c) -knownf(r,c-1))) > (abs(source_I(x,y) -source_I(x,y-1)))
                Vpq3 = knownf(r,c) -knownf(r,c-1);
            else
...
figure;
imshow(imgresult);
title('Result of Task2b-mixing gradient Seamless cloning');
